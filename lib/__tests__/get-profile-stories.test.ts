...
beforeEach(async () => {
  ...
  const p1 = await db.profile.create({
    data: { name: 'Zara', ageRange: '1-2y', avatar: {}, userId },
  })
  const p2 = await db.profile.create({
    data: { name: 'Leo', ageRange: '2-3y', avatar: {}, userId },
  })
  const p3 = await db.profile.create({
    data: { name: 'Mia', ageRange: '0-12m', avatar: {}, userId },
  })
  profileId1 = p1.id
  profileId2 = p2.id
  profileId3 = p3.id
  ...
  it('returns stories for a profile that is one of several co-profiles', async () => {
    await db.story.create({
      data: {
        ...
        lesson: 'patience',
        body: 'Once...',
        coProfileIds: [profileId2, profileId3],
        saved: true,
      },
    })
